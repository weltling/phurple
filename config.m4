...
if test "$PHP_PHURPLE" != "no"; then
	...
	[
		...
		PHP_ADD_INCLUDE($GLIB_DIR/$PHP_LIBDIR/glib-2.0/include)
		PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $GLIB_DIR/$PHP_LIBDIR, GLIB_SHARED_LIBADD)
		AC_DEFINE(HAVE_GLIBLIB,1,[ ])
	],[
		AC_MSG_ERROR([wrong glib lib version or glib not found])
	],[
		-L$GLIB_DIR/$PHP_LIBDIR -lglib-2.0
	])
	...
	[
		...
		PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $PHURPLE_DIR/$PHP_LIBDIR, PHURPLE_SHARED_LIBADD)
		AC_DEFINE(HAVE_PHURPLELIB,1,[ ])
	],[
		AC_MSG_ERROR([wrong purple lib version or lib not found])
	],[
		-L$PHURPLE_DIR/$PHP_LIBDIR -lpurple
	])
